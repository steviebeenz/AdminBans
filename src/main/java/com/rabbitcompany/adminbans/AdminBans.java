...
public final class AdminBans extends JavaPlugin {
  ...
  public void onEnable() {
    ...
    // Commands
    this.getCommand("adminbans")
        .setExecutor(new com.rabbitcompany.adminbans.commands.AdminBans());
  ...
  private void setupMySQL() {
    try {
      ...
      hikari.setJdbcUrl("jdbc:mysql://" + getConf().getString("mysql_host") +
                        ":" + getConf().getString("mysql_port") + "/" +
                        getConf().getString("mysql_database"));
      hikari.setUsername(getConf().getString("mysql_user"));
      hikari.setPassword(getConf().getString("mysql_password"));
      hikari.addDataSourceProperty("useSSL",
                                   getConf().getString("mysql_useSSL"));
  ...
  private void info(String message) {
    ...
    Bukkit.getConsoleSender().sendMessage(
        Message.chat("&6[]=====[" + message + " &cAdminBans&6]=====[]"));
    Bukkit.getConsoleSender().sendMessage(Message.chat("&6|"));
    Bukkit.getConsoleSender().sendMessage(Message.chat("&6| &cInformation:"));
    Bukkit.getConsoleSender().sendMessage(Message.chat("&6|"));
    Bukkit.getConsoleSender().sendMessage(
        Message.chat("&6|   &9Name: &bAdminBans"));
    Bukkit.getConsoleSender().sendMessage(
        Message.chat("&6|   &9Developer: &bBlack1_TV"));
    if (!username.contains("%%__")) {
      Bukkit.getConsoleSender().sendMessage(
          Message.chat("&6|   &9Plugin owner: &b" + username));
    } else if (!user_id.contains("%%__")) {
      Bukkit.getConsoleSender().sendMessage(
          Message.chat("&6|   &9Plugin owner: &b" + user_id));
    } else {
      Bukkit.getConsoleSender().sendMessage(
          Message.chat("&6|   &9Plugin owner: &4&lCRACKED"));
    }
    Bukkit.getConsoleSender().sendMessage(
        Message.chat("&6|   &9Version: &b1.1.0"));
    Bukkit.getConsoleSender().sendMessage(Message.chat("&6|"));
    Bukkit.getConsoleSender().sendMessage(Message.chat("&6| &cSupport:"));
    Bukkit.getConsoleSender().sendMessage(Message.chat("&6|"));
    Bukkit.getConsoleSender().sendMessage(
        Message.chat("&6|   &9Discord: &bCrazy Rabbit#0001"));
    Bukkit.getConsoleSender().sendMessage(
        Message.chat("&6|   &9Mail: &bziga.zajc007@gmail.com"));
    Bukkit.getConsoleSender().sendMessage(
        Message.chat("&6|   &9Discord: &bhttps://discord.gg/hUNymXX"));
    Bukkit.getConsoleSender().sendMessage(Message.chat("&6|"));
    Bukkit.getConsoleSender().sendMessage(
        Message.chat("&6[]=====================================[]"));
    Bukkit.getConsoleSender().sendMessage(Message.chat(""));
