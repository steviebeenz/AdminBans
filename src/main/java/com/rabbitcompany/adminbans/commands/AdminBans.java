  ...
  public boolean onCommand(CommandSender sender, Command command, String label,
    ...
    if (!(sender instanceof Player)) {
      ...
      if (args.length == 1) {
        ...
        case "import":
          sender.sendMessage(
              Message.chat("&cPlease use /adminbans import <bukkit>"));
          break;
        default:
          sender.sendMessage(
              Message.getMessage(UUID.randomUUID(), "adminbans_syntax"));
          break;
        }
        ...
        case "bukkit":
          ...
          for (BanEntry banned :
            ...
            if (banned.getExpiration() != null) {
              AdminBansAPI.banPlayer(
                  "Console", "Console",
                  Bukkit.getOfflinePlayer(banned.getTarget())
                      .getUniqueId()
                      .toString(),
                  banned.getTarget(), banned.getReason(),
                  AdminBansAPI.date_format.format(banned.getExpiration()));
            } else {
              Date until = new Date(System.currentTimeMillis() + 315569520000L);
              AdminBansAPI.banPlayer("Console", "Console",
                                     Bukkit.getOfflinePlayer(banned.getTarget())
                                         .getUniqueId()
                                         .toString(),
                                     banned.getTarget(), banned.getReason(),
          ...
          break;
        default:
          sender.sendMessage(Message.chat("&cAdminBans don't support &a" +
                                          args[1] + "&c ban plugin."));
          break;
        }
      } else {
    ...
    if (player.hasPermission("adminbans")) {
      if (args.length == 1) {
        ...
        case "help":
          player.sendMessage(
              Message.getMessage(player.getUniqueId(), "commands"));
          break;
        case "reload":
          if (player.hasPermission("adminbans.reload")) {
            com.rabbitcompany.adminbans.AdminBans.getInstance().loadYamls();
            player.sendMessage(
                Message.getMessage(player.getUniqueId(), "reload"));
          } else {
            player.sendMessage(
        ...
        default:
          player.sendMessage(
              Message.getMessage(player.getUniqueId(), "adminbans_syntax"));
          break;
        }
      } else {
        player.sendMessage(
